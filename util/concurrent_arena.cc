...
SimpleConcurrentArena::SimpleConcurrentArena(bool disableConcurrency,
                                             size_t block_size,
  ...
  for (int i = 0; i < nConcurrentAreans_; i++) {
    Arena* p = new Arena(OptimizeBlockSizeForConcurrentArena(block_size),
                         huge_page_size);
...
char* SimpleConcurrentArena::AllocateAligned(size_t bytes,
                                             size_t huge_page_size,
                                             Logger* logger) {
  int i = 0;
  if (nConcurrentAreans_ > 1) {
  ...
  if (disableConcurrency_) {
    return pDescriptors_[i].pArena->AllocateAligned(bytes, huge_page_size,
                                                    logger);
  } else {
    MutexLock l(pDescriptors_[i].pMutex);
    return pDescriptors_[i].pArena->AllocateAligned(bytes, huge_page_size,
