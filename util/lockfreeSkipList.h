...
// This skip-list is a variant of the algorithm in Chapter 14.4 of the book: The
// Art of Multiprocessor Programming, 2008, Herlihy and Shavit.
// It does not support delete operations, and includes iterators that guarantee
// weak-consistency.
...
class LockFreeSkipList {
 private:
  ...
  struct Node {
    ...
    TItem value;
    int topLevel;
    std::atomic<Node*> next[1];  // the actual size of this array is height+1
                                 // (allocated externally)
  };

  // Random level generator
  // Inspired by the random level generator from Doug Lea's
  // ConcurrentSkipListMap.java
  // (based on a pseudo-random function that was used in turbo pascal)
  class RandomLevelGenerator {
   public:
    RandomLevelGenerator() {
      currentState_.store((unsigned int)Env::Default()->NowMicros(),
                          std::memory_order_relaxed);
    }
    int nextRandomLevel() {
      unsigned int num =
          currentState_.load(std::memory_order_relaxed) * 134775813 + 1;
 ...
 public:
...

  {
    char* mem = allocator_->AllocateAligned(
        sizeof(Node) + MAX_LEVEL * sizeof(std::atomic<Node*>));
    pTail_ = new (mem) Node((TItem) nullptr, MAX_LEVEL);

    mem = allocator_->AllocateAligned(sizeof(Node) +
  ...
  bool insert(TItem x) {
    ...
    while (true) {
      ...
      if (found) {
        ...
        if (!pred->next[0].compare_exchange_strong(succ, newNode,
        ...
        for (int level = 1; level <= topLevel; level++) {
          while (true) {
            ...
            if (pred->next[level].compare_exchange_strong(
                    succ, newNode, std::memory_order_release,
                    std::memory_order_relaxed))
              break;
            find(x, preds, succs, false);
          }
  ...
  class Iterator {
   ...
   public:
    ...
    void SeekToFirst() { current_ = pList_->pHead_->next[0]; }
