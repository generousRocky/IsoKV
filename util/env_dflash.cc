...
static int DFlashLockOrUnlock(const std::string& fname, dflash_file *fd,
                              bool lock) {
  mutex_lockedFiles.Lock();
  if (lock) {
    // If it already exists in the lockedFiles set, then it is already locked,
...
static bool ParseType(std::string fname, FileType* type) {
  uint64_t num;
  size_t dir_pos;
  std::string filename;
...
class DFlashEnv : public Env {
 public:
  DFlashEnv() :
    ...
    ALLOC_CLASS(root_dir, dflash_dir("root", 4, nvm_api, nullptr));
    LoadFtl();
  }
  ...
  virtual Status SaveFTL() override {
    ...
    dflash_file *current = root_dir->file_look_up(manifest_path.c_str());
    if (current == nullptr) {
      return Status::IOError("Cannot retrieve %s", current_manifest);
    }

    std::string manifest_meta;
    DFlash_DEBUG("SAVING METADATA!!!!!\n");
    void* meta = DFlashPrivateMetadata::GetMetadata(current);
  ...
  virtual Status NewRandomAccessFile(const std::string& fname,
    ...
    if (type == kInfoLogFile || type == kCurrentFile ||
      ...
      result->reset(new DFlashRandomAccessFile(fname, f, root_dir));
      return Status::OK();
    }
  }
  ...
  virtual Status NewWritableFile(const std::string& fname,
    ...
    if (type == kInfoLogFile ||
      ...
      if (fd == nullptr) {
        *result = nullptr;
        DFlash_DEBUG("unable to open file for write %s", fname.c_str());
        return Status::IOError("unable to open file for write");
      }

      DFlashWritableFile *writable_file;
      ALLOC_CLASS(writable_file, DFlashWritableFile(fname, fd, root_dir));
  ...
  virtual Status NewDirectory(const std::string& name,
    ...
    // DFlash
    dflash_dir *fd = root_dir->OpenDirectory(name.c_str());
    if (fd == nullptr) {
      DFlash_DEBUG("directory %s not found", name.c_str());
      return Status::IOError("directory doesn't exist");
    }

    result->reset(new DFlashDirectory(fd));
  ...
  virtual Status FileExists(const std::string& fname) override {
    ...
    if (type == kInfoLogFile ||
      ...
      if (exists) {
        DFlash_DEBUG("%s exists: %d", fname.c_str(), exists ? 1 : 0);
        return Status::OK();
      } else {
        return Status::NotFound();
      }
  ...
  virtual Status GetFileModificationTime(const std::string& fname,
    ...
    if (type == kInfoLogFile || type == kCurrentFile ||
      ...
      *file_mtime = static_cast<uint64_t>(s.st_mtime);
      ...
      if (root_dir->GetFileModificationTime(fname.c_str(),
                                                  (time_t *)file_mtime)) {
  ...
  virtual Status RenameFile(const std::string& src,
    ...
    DFlash_DEBUG("renaming %s to %s", src.c_str(), target.c_str());
    FileType type;
    ...
    if (type == kInfoLogFile || type == kCurrentFile ||
      ...
      DFlash_DEBUG("Failed to rename %s to %s", src.c_str(), target.c_str());
  ...
  virtual Status LockFile(const std::string& fname, FileLock** lock) override {
    ...
    *lock = nullptr;
    if (type == kInfoLogFile || type == kCurrentFile ||
      ...
      DFlashFileLock *my_lock = new NVMFileLock;
      my_lock->fd_ = f;
      my_lock->filename = fname;
  ...
  virtual Status NewLogger(const std::string& fname,
    ...
    if (f == nullptr) {
      ...
      int fd = fileno(f);
      SetFD_CLOEXEC(fd, nullptr);
      result->reset(new PosixLogger(f, &DFlashEnv::gettid, this));
      return Status::OK();
    }
  }
  ...
  EnvOptions OptimizeForCurrentWrite(
      const EnvOptions& env_options) const override {
    EnvOptions optimized = env_options;
  ...
  void LoadSuperblockMetadata(std::string* fname, Slice* super_meta) {
    ...
    if (LoadPrivateMetadata(*fname, meta) != Status::OK()) {
      DFlash_DEBUG("Could not load superblock metadata\n");
    }
  }
...
next_meta:
  ...
  Status LoadPrivateMetadata(std::string fname, void* metadata) {
    ...
    if (metadata == nullptr) {
      DFlash_DEBUG("Metada for %s not in MANIFEST\n", fname.c_str());
      // Try reading metadata from DFLassh private metadata (metadata that did
      // not make it to the MANIFEST before last instance was closed.
      if (!fd->LoadSpecialMetadata(fname)) {
        DFlash_DEBUG("Metadata not loaded for file: %s\n", fname.c_str());
        return Status::IOError("Metadada not loaded for file");
      }
      DFlash_DEBUG("Metadata loaded: %s\n", fname.c_str());
      return Status::OK();
    }

    if (!fd->LoadPrivateMetadata(fname, metadata)) {
      DFlash_DEBUG("Metadata not loaded for file: %s\n", fname.c_str());
      return Status::IOError("Error while loading metadata");
    }
    DFlash_DEBUG("Metadata loaded: %s\n", fname.c_str());
    return Status::OK();
  }
  ...
  Status LoadPrivateMetadata(std::string fname) override {
    ...
    if (fd->metadata_handle_ == nullptr) {
      return Status::IOError("Error loading metadata");
    }

    void* metadata = fd->metadata_handle_->GetPrivateMetadata();
    if (metadata == nullptr) {
      DFlash_DEBUG("Metada for %s not in MANIFEST\n", fname.c_str());
      // Try reading metadata from DFLassh private metadata (metadata that did
      // not make it to the MANIFEST before last instance was closed.
    ...
    if (!fd->LoadPrivateMetadata(fname,
                                 fd->metadata_handle_->GetPrivateMetadata())) {
      DFlash_DEBUG("Metadata not loaded for file: %s\n", fname.c_str());
      return Status::IOError("Error while loading metadata");
    }
    DFlash_DEBUG("Metadata loaded: %s\n", fname.c_str());
    fd->metadata_handle_->FreePrivateMetadata();
    return Status::OK();
  }

  // Static method encoding WAL metadata for DFlash backend
  // TODO: Get WAL dir from dboptions
  void EncodeLogPrivateMetadata(std::string* dst, uint64_t log_number,
                                uint8_t priv_type) {
    std::string fname = LogFileName("testingrocks", log_number);
    dflash_file* fd = root_dir->file_look_up(fname.c_str());
    if (fd == nullptr) {
      DFlash_DEBUG("Log file %s not found for saving metadata\n", fname.c_str());
      return;
    }
  ...
  void DiscoverAndLoadLogPrivateMetadata(uint64_t log_number) {
    ...
    dflash_file* file = root_dir->file_look_up(log_name.c_str());
    if (file != nullptr) {
      if (file->HasBlock()) {
        DFlash_DEBUG("File %s already loaded\n", log_name.c_str());
        return;
      }
    }

    file->LoadSpecialMetadata(log_name);
  }
  ...
  void DecodeAndLoadLogPrivateMetadata(Slice* input, uint64_t log_number) {
    ...
    free(vblock_meta);
    DFlash_DEBUG("Log file %s not found for loading metadata\n", fname.c_str());
  }
 ...
 private:
  ...
  void LoadFtl() {
    ...
    if (fd < 0) {
      DFlash_DEBUG("FTL file not found");
      return;
    }

    if (read(fd, &temp, 1) != 1) {
      DFlash_DEBUG("FTL file is corrupt");
      close(fd);
      return;
    }
    ...
    DFlash_DEBUG("read d. loading root directory");
...
Env* Env::Default() {
  static DFlashEnv default_env;
  return &default_env;
}

// Static method encoding sstable metadata for DFlash backend
// See comment above DFlash:PrivateMetadata::GetMetadata()
void Env::EncodePrivateMetadata(std::string *priv, void *metadata) {
  ...
  // DFlashPrivateMetadata::GetMetadata()
  struct vblock_meta *vblock_meta = (struct vblock_meta*)metadata;
  priv->append((const char*)vblock_meta->encoded_vblocks, vblock_meta->len);
  return;
}
...
void* Env::DecodePrivateMetadata(Slice* input) {
  ...
  while (left > 0) {
    ...
    if (meta32 != DFlashPrivateMetadata::separator_) {
      DFlash_DEBUG("Private metadata from manifest is corrupted\n");
      return nullptr;
    }
    ...
    DFlash_DEBUG("Decoding: id: %lu, ownerid: %lu, nppas: %lu, ppa_bitmap: %lu, bppa: %llu, vlun_id: %d, flags: %d\n",
